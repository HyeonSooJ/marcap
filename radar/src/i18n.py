...
TRANSLATIONS = {
    ...
    'tab5_presurge_note': {
        'ko': (
            ...
            '해당하면 그중 하나만 표시됩니다. 아직 상한가가 확정되지 않은 종목 중 모양만으로 후보를 '
            '추정하는 용도입니다.  \n'
            '"눌림목형"/"눌림목 반등형"은 조회 기간 전체가 아니라 최근 6거래일만 보고 "짧게 오르고-'
            '살짝 눌리고-아직 저점보다는 높게 마감"인 종목을 찾습니다(2026-09-04 실제 급등주 '
            'TPC로보틱스·E8 사례로 확인 — 이 둘은 전체 기간 모양 비교로는 안 잡혔음). 둘의 차이는 '
            '"저점매수" 타이밍입니다 — **눌림목형**은 오늘 종가가 눌림목 저점 그 자체라 아직 반등이 '
            '시작되지 않은 상태(내일 저점매수 후보), **눌림목 반등형**은 이미 그 저점에서 살짝 '
            '올라온(반등이 시작된) 상태입니다. 또한 마지막 날 캔들의 위꼬리/아래꼬리 중 긴 쪽이 몸통의 '
            '2배를 넘으면(방향성이 불분명한 도지형 등) 두 유형 모두에서 제외합니다.  \n'
        ...
        'en': (
            '⑤ Pre-surge Pattern combines five sub-conditions (Sideways-then-Breakout, '
            'Bottom-then-Rebound, Pullback-at-the-low, Pullback-Rebound, Rally-then-Pullback). The '
            ...
            'candidates estimated from chart shape alone.  \n'
            '"Pullback-at-the-low" / "Pullback-Rebound" look only at the last 6 trading days (not the '
            'full period) for "rose briefly, pulled back a bit, still closing above the pullback low" '
            '(confirmed against real 2026-09-04 surging stocks TPC Robotics and E8, which the '
            'full-period shape comparisons missed). The difference between the two is buy-the-dip '
            'timing — **Pullback-at-the-low** means today\'s close IS the pullback low itself (no '
            'bounce yet, a candidate to buy tomorrow), while **Pullback-Rebound** means it has '
            'already bounced slightly off that low. Both exclude stocks whose last-day candle has an '
            'upper or lower wick more than 2x its body (an indecisive, doji-like candle).  \n'
    ...
    'tab5_presurge_disclaimer': {
        ...
        'en': (
            '⚠️ All five conditions were walk-forward backtested on 2026 data, with mixed results. '
            ...
            '(-8.2%). **Pullback-at-the-low / Pullback-Rebound** (after the wick filter): next-day '
            're-hit probability was 1.21%/1.09% respectively (vs. 0.55%/0.62% random, ~2.2x/1.8x) — a '
            'weak but consistent signal (validated on 105 test dates, Apr-Sep 2026). '
            '**Rally-then-Pullback**: average 1-month return was -13.2%, only marginally better than '
            'random (-15.7%). None of the five guarantees a limit-up, and none is suited for '
