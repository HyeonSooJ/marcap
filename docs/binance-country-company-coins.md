...
| FWDIUSDT | 포워드인더스트리즈(Forward Industries) | 미확인 |
...
| BBXUSDT | 블랙베리(BlackBerry Limited) | 미확인 |
...
| EWJUSDT | iShares MSCI Japan ETF(일본 지수) | 미확인 | 미확인 |
...
| EWTUSDT | iShares MSCI Taiwan ETF(대만 지수) | 미확인 | 미확인 |

### 🇧🇷 브라질 (한국인 거래 가능)

| 티커 | 기초자산 | 최대 레버리지 |
|---|---|---|
| EWZUSDT | iShares MSCI Brazil ETF(브라질 지수) | 미확인 |

**중요 발견**: `EWY`(한국)·`EWJ`(일본)·`EWT`(대만)·`EWZ`(브라질)는 전부
**iShares MSCI 국가별 ETF 시리즈**입니다. 즉 사용자가 처음 예로 든 EWY는
개별 상품이 아니라 이 시리즈의 한국 버전이었던 것 — 바이낸스가 이
시리즈를 국가별로 여러 개 상장해둔 것으로 보입니다. 다른 국가 버전
...
> **업데이트(재확인 결과)**: `SPCXUSDT`는 더 이상 "비상장" 기업이
> 아닙니다. **SpaceX가 2026-06-12 나스닥에 실제 상장(티커 SPCX, 공모가
> $135, 첫날 종가 $160.95, 시총 약 2.1조 달러)**을 완료했습니다.
> 바이낸스 정책상 기초자산이 실제 상장되어 안정적인 시세 산정이
> 가능해지면 Pre-IPO 계약을 **일반 TradFi 무기한 선물로 전환**한다고
> 명시하고 있어, SPCXUSDT는 현재 TSLAUSDT처럼 실제 나스닥 시세를
> 추종하는 일반 상품일 가능성이 높습니다(2026-09-12 기준 여전히
> 활발히 거래 중, 가격 약 $148~153). `OPENAIUSDT`·`ANTHROPICUSDT`는
> 2026-09-12 기준 재확인 결과 **여전히 Pre-IPO 상태로 정상 거래
> 중**이며(오픈AI·앤트로픽 모두 아직 실제 IPO 전), 상장폐지되지
> 않았습니다.
...
- SpaceX(미국) — `SPCXUSDT`(USDT 결제)는 **2026-05-21** 최초 상장(세계
  ...
  추가 상장. **2026-06-12 SpaceX 나스닥 실제 상장**으로 현재는 Pre-IPO가
  아닌 일반 주식 추종 계약으로 전환된 것으로 추정(재확인 필요).
- OpenAI(미국, `OPENAIUSDT`) — **2026-05-26 08:30 UTC** 출시, Pre-IPO
  Perpetual Contracts 계열 두 번째 상품. 레버리지 미확인. 2026-09-12
  기준 여전히 Pre-IPO 상태로 정상 거래 중(오픈AI는 2026년 9월 중
  IPO 가능성이 보도되었으나 이번 조사 시점까지 미상장).
- Anthropic(미국, `ANTHROPICUSDT`) — **2026-06-02 04:30 UTC** 출시,
  최대 20배 레버리지. 앤트로픽이 SEC에 비공개 S-1을 제출한 지 몇 시간
  만에 상장된 것으로 보도됨. 2026-09-12 기준 여전히 Pre-IPO 상태로
  정상 거래 중(가격 $2,100대, 내재 시총 약 2.1조 달러). 앤트로픽
  실제 IPO 공모 마케팅은 2026년 10월 중순 이후로 보도됨.
